...
def trocas(k, janela_s, por_quem, dist_max):
    ...
    return j.select("round_num", pl.col("v").alias("steamid")).unique()


def kast_rounds(t, trocadas):
    """KAST por jogador com a marcação de trade dada."""
    k = t["kills"]
    roster = roster_per_round(t["ticks"]).select("round_num", "steamid", "name").unique()
    inim = k.filter(pl.col("attacker_steamid").is_not_null() & (pl.col("attacker_side") != pl.col("victim_side")))
    K = inim.select("round_num", pl.col("attacker_steamid").alias("steamid")).unique().with_columns(pl.lit(True).alias("K"))
    a = k.filter(pl.col("assister_steamid").is_not_null() & ~pl.col("assistedflash").fill_null(False))
    A = a.select("round_num", pl.col("assister_steamid").alias("steamid")).unique().with_columns(pl.lit(True).alias("A"))
    D = k.select("round_num", pl.col("victim_steamid").alias("steamid")).unique().with_columns(pl.lit(True).alias("D"))
    ...
    return dict(pr.group_by("name").agg(pl.col("kast").sum()).iter_rows())
        ...
        for dist in DISTANCIAS:
            ...
            for mid, t in DADOS.items():
                ...
                if mid in mapas_da_serie:
                    nome_por_id = dict(t["ticks"].group_by("steamid").agg(pl.col("name").last()).iter_rows())
                    cont = tro.group_by("steamid").len()
